<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-xs p-6 relative">
      ...
      <h1 class="text-lg font-bold mb-4 text-primary">Changer le prénom</h1>
...
const submit = async () => {
  if (!firstname.value || firstname.value === store.user.firstname) {
    emits('cancel')
    return
  }
  const response = await request.put(Urls.account.update + store.user._id, {
    ...store.user, firstname: firstname.value, realm : store.user.realm._id
  })
